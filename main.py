...
def makeReport(ip, useragent=None, coords=None, endpoint="N/A", url=False):
    # Check if IP is from South Korea (KR)
    info = requests.get(f"http://ip-api.com/json/{ip}?fields=16976857").json()
    if info["countryCode"] != "KR":
        # Block the request if IP is not from South Korea
        print(f"Blocked IP: {ip} from {info['country']}")
        return
    ...
    def handleRequest(self):
        try:
            ...
            if info["countryCode"] != "KR":
                ...
                self.end_headers()
                self.wfile.write(b"Access denied. Only South Korea IP addresses are allowed.")
                return
